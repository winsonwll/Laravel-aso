...
class TaskController extends Controller
{
    ...
    public function distributeTask(Request $request)
    {
        ...
        if(!empty($task)){
            ...
            if(!empty($account)){
                Task::where('tid', $task->tid)->increment('vcnt',1);      //更新请求数
                Account::where('aid', $account->aid)->update(['state' => 1, 'cid'=> $cid]);   //账号状态 0 未使用  1 使用中  2 已使用  3 账号禁用  4 登录失败
    ...
    public function finishTask(Request $request)
    {
        ...
        if($data['status'] == 200){      //任务成功
            ...
            if ($data['status'] == 205){
                Account::where('aid',$aid)->update(['state' => 3]);
            }elseif ($data['status'] == 212 || $data['status'] == 204){
                Account::where('aid',$aid)->update(['state' => 4]);
            }else{
                Account::where('aid',$aid)->update(['state' => 0]);
            }
